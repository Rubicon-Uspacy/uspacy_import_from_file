...
def fetch_fields(
    session: requests.Session, base_url: str, entity: str
) -> Dict[str, FieldInfo]:
    url = f"{base_url}/crm/v1/entities/{entity}/fields"
...
def search_entity(
    ...
    url = f"{base_url}/crm/v1/entities/{entity}/"
    params = {
...
def patch_entity(
    ...
    url = f"{base_url}/crm/v1/entities/{entity}/{entity_id}"
...
def main() -> int:
    ...
    fields_info = fetch_fields(session, args.base_url.rstrip("/"), args.entity)
    ...
    for index, row_values in enumerate(rows_iter, start=2):
        ...
        matches = search_entity(
            session, args.base_url.rstrip("/"), args.entity, search_field, search_value
        )
        if not matches:
        ...
        patch_entity(session, args.base_url.rstrip("/"), args.entity, entity_id, payload)
